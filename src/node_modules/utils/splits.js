  ...
  return new Promise((resolve, reject) => {
    try {
      require.ensure([
        ...
        const SettingsStore =
          require('application/stores/SettingsStore').default;

        app.registerStore(SettingsStore);

        const settings = {
          action: require('application/actions/settings').default,
          component: require('application/components/pages/settings').default
        };

        // Inject action into the actions interface for backend availability.
        actionsInterface.putAction(payload.action.name, settings.action);
